const data = [
    ...
    <p>
    <i class="vurgu">Bir felsefi disiplin olarak postmodernizm; kitlelerin/insanın ürettiği anlam değişimlerini inceleyen analiz biçimidir. </i>
    Üretilen anlam vurgusu önemli çünkü temelini dil felsefesinden alır. Objeler dildeki kelimelerle etiketlendiğinde anlam kazanırlar. Biz anlamlı şeyleri
    gerçekliğimize dahil ederiz. Dil ile gerçekliği yaratırız. Mesela yer çekiminin varlığı biz yer çekimi olarak adını koyduğumuzda bizim için var olmaya başladı.
    Mikrop konsepti ortaya çıkmadan önce de bizim için gerçek değildi.("gerçek" kısmına eleştiriler kısmında daha kapsamlı değineceğim.) 
    Üretilen bu anlam değişimleri iterasyonlarla ilerlemez. Anlamlandırılmış şeyin bir önceki ya da bir sonraki anlamı birbirleri ile alakasızdır. Tekrar 
    yer çekimi örneğine dönecek olursak; Newton fiziğinde bir kuvvetiyken Einstein'ın genel görelilik kuramında uzayzamanda oluşan geometrik bir bükülmedir.
    Bu iki kuram yer çekimi olarak adlandırdığımız olguyu bambaşka şekillerde ele almaktadır. Bir iterasyondan ziyade Thomas Kuhn'un tabiriyle paradigma değişimi
    söz konusudur. </p>



    <p> <i class="vurgu">Peki bu anlamlar nasıl değişmektedir?</i>
    Postmodernizm'in özü bu soruda yatmaktadır. Bir kültür teorisi olan poststructuralism ve Göstergebilimde bilinen
    Semiyotik Üçgen'de gösterilebilmektedir. Poststructuralism, structuralism'e yani yapısalcılığa tepki olarak çıkan akımdır. Yapısalcılık kelimeleri tanımlamanın yolunun
    o kelimeyi olmadığı diğer kelimelerle tanımlamasıdır. Örneğin insan kelimesi tek başına bir şey ifade etmez. İnsan olmayan hayvan, bitki ya da cansız varlıklar ile
    karşılaştırılmasıyla, benzerlikleriyle bir anlam elde eder. İnsan konsepti diğer konsept yapılarından elde edilir. Bu örnekten hareketle daha geniş bir perspektiften; 
    ...
    <p>
    Semiyotik üçgende gönderen, alıcı ve sembol(ortam ve içeriği de temsil edecek şekilde) 
    bir üçgen içerisinde birbirlerini etkilemektedir. Gönderen anlamı alıcıya ve ortama uygun bir şekilde, içeriğin(video, makale, görsel) içine paketlediğinde ölmekte, alıcı 
    içeriği anlamlandırırken göndereni ve içeriği yeniden yaratmak zorunda kalmaktadır. Ortam ise alıcı ve göndereni direkt olarak etkilemektedir. Dillere pelesenk olan 
    "Ortam mesajın kendisidir." ifadesi gözlemlenmiş bir çıktıdır. Bir pisuar tuvalette bir pisuarken sanat galerisinde bir sanat objesidir. Bir çay tabağı yerine göre bir çay tabağı yerine göre bir küllük
    olabilir. Bir cümle arkadaş ortamında kullanıldığında eğlenceli bir şakayken bir sınıfta patavatsızlık olarak görülebilir. Gönderen, alıcılar ve ortam değişmiştir dolayısıyla
    anlam da değişmiştir. 
    </p>
      
    <h3>Eleştiriler</h3>
    <i class="vurgu">120 tonluk bir aracın içinde 10.000 metre yükselikteyken postmodernist olamazsınız.</i>
    <p> Postmodernizmde gerçeğe atıf yapıldığında kastedilen gerçekliğin olmadığı değil bağlamdan bağımsız bir gerçeğin olmadığıdır. 
    Vurgulanan nokta yer çekimi ya da mikrobun var olup olmaması değildir. Dil ile belirli bir bağlamda oluşturularak algılanmasıdır.  
    Bu gözlemi ilk fark edenler arasında Kant Dünya'yı filtrelerden bağımsız göremeyeceğimizi söylemiştir. Bu noktada bilgi felsefesi açısından Türkçede bilimsel 
    objektiflik olarak geçen Objective Truth tanımını bilmekte yarar var. Bilimsel objektiflik insan/toplum ön yargısından ve çıkarlarından bağımsız sonuçları temsil eder. 
    Postmodernizm gerçekliği reddetmez; bilimsel objektifliği yani nesnelliği eleştirir. 
    </p>
    <i class="vurgu">Postmodernizm her yeni teori çıktığında bir öncekini reddediyor. gerçek birşey olmadığını savunuyor.</i>
    <p>
     Postmodernizm, genel görelilik kanunu kabul ediliyor diye Newton fiziğini reddetmez. Sonuçta sadece Newton fiziği kullanarak Ay'a roket inirebileceğimiz
     gerçeğini reddedemeyiz. İki yasada aynı anda doğrudur ve birbirlerine indirgenemezler. Bu açıdan Plüralist bir yaklaşımı vardır. 
     Postmodernizm 'Büyük Resmi' eleştirir. Her şeyi etrafında şekillendirdiğimiz o öyküyü reddeder. Örnek olarak; Karl Marx'ın bütün dünya düzenini
    sınıflar arası çatışma ile açıklaması, Hitler'in üstün Alman ırkının Dünya'yı yönetmesi gerektiği inancı verilebilir.
    </p>
    <i class="vurgu">Postmodernizm relativismi savunur. Bu nedenle konsept kendi kendini yok ediyor. </i>
    <p>
    Relativism(Görecelilik) kendi kendini yok eden bir konsept olduğu doğrudur. Her şey göreceliyse  her şeyin göreceli olduğu argümanı da görecelidir. Görüş kendi
    kendisini dışlıyor fakat postmodernizm ile alakası yoktur. Evrenin farklı gerçekleri çalıştırdığını kabul etmek ile her şeyin göreceli olduğunu
    kabul etmenin aynı kefeye koyulması sonucu çıkan yanlış anlaşılmadır. Ayrıca farkı postmodernizm bir argüman ya da bir tepki olarak çıkmamıştır. Yukarıda anlattığım gibi
    dilde anlamın nasıl değiştini incelerken fark edilmiş bir gözlem/analiz biçimidir.
    </p>

    <h3>Sonuç</h3>
    <p>
      Konsept hakkındaki fikirlerimi düzenleyerek açıkladığım bu yazının sonuna geldim. Metin boyunca örnekleri ağırlıklı olarak sosyal bilimler üzerinden vermedim. Benim için
      iki sebebi var. Birincisi pozitif bilimlerdeki etkisini kavraması çok zorlu bir süreçti ve aylarımı aldı. İkincisi postmodern durum altında "bizi kandırıyorlar" 
      tarzı söylemleri anımsatmasından çekindim. Jean Baudrillard'in Disneyland veya Körfez Savaşı incelemeleri örnek verilebilirdi fakat ne kadar
      eşsiz analizler olsa da kıta felsefesinin dramatik dilinden yoruldum. İlerleyen yazılarımda zaten postmodern ontoloji üzerinden felsefe yapmayı
      düşündüğüm için yeterli bir temel olarak değerlendiriyorum.
    </p>
